class BouldersController < ApplicationController
  ...
  def params_valid?
    boulder_params[:session_climbs_attributes].nil? || (
      session_climbs_present? && boulder_params[:session_climbs_attributes].size < 2
    )
  end

  def session_climbs_present?
    boulder_params.present? && (
      boulder_params[:session_climbs_attributes].present? &&
  ...
  def set_session_climb
    return unless session_climbs_present? && @boulder&.persisted?
